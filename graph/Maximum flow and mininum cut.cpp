...
int main() {
	matrix(int) amatrix(n + 1, vi(n + 1, 0)); /*adjacency list of the graph, index start from 1*/
	matrix(int) residue = amatrix; /*a copy of adjacecy list*/
	int source, sink;
	int mf=maxflow(residue, source, sink); /*return the maximum flow (also the capcity of the mininum cut),
    residue will become the adjacency matrix residue network after subtracting the maximumflow*/
}